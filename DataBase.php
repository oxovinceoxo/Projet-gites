 ...
 ?>
    ...
    public function filtrePrix () {
        ...
        if (isset($_POST['prix'])) {
            ...
            if (!empty($_POST["NbrChambre"])) {
                $reponse = $BD->prepare("SELECT * FROM type WHERE nbr_chambre=? ORDER BY prix ASC");
                $reponse->bindParam(1, $_POST["NbrChambre"]);
                $reponse->execute(array($_POST["NbrChambre"]));
                ...
                echo "nombre de chambres est vide";
                var_dump($_POST["NbrChambre"]);
            }

            if($prix == 2) {
                $reponse = $BD->query("SELECT * FROM type WHERE nbr_chambre=? ORDER BY prix DESC");
                $reponse->bindParam(1, $_POST["NbrChambre"]);
                $reponse->execute();
...
// 6 j'affiche mon element avec fetch (pour charger les resultats)
        ...
        if($resultat){
            ...
            <a href="email.php?id_gite=<?= $resultat['id_gite']?>" >RESERVER</a>
...
// 6 j'excute la requete
    ...
    public function GiteNonDispo () {

        $id_gite=$_GET["id_gite"];
        $BD= $this->databaseConnexion();
        $sql="UPDATE `type` SET `disponible`=0 WHERE id_gite=? ";
        $GitIndispo = $BD->prepare($sql);
        $GitIndispo->bindParam(1,$id_gite);
        $fini=$GitIndispo->execute();
        if($fini){
            header("refresh:2, http://localhost/Gites3/client.php");
        }else{
            echo "votre reservation a echouée";
        }
